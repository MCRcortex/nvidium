...
void main() {
    ...
    OUT[(gl_LocalInvocationID.x<<1)|0].uv_bias = vec3(vec2(A.g,A.h)*(1f/65536), (material&1u)!=0u?0.0f:-4.0f);//Temporary untill sodium 0.5
    OUT[(gl_LocalInvocationID.x<<1)|1].uv_bias = vec3(vec2(B.g,B.h)*(1f/65536), (material&1u)!=0u?0.0f:-4.0f);//Temporary untill sodium 0.5
