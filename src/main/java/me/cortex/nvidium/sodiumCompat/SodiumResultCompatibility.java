...
public class SodiumResultCompatibility {
    ...
    private static long getVertexDataAddress(BuiltSectionMeshParts mesh) {
        return MemoryUtil.memAddress(mesh.getVertexData().getDirectBuffer());
    }
    ...
    private static void packageSectionGeometry(int formatSize, NativeBuffer output, short[] outOffsets, ChunkBuildOutput result, Vector3i min, Vector3i max) {
        ...
        if (translucentData != null) {
            int vertexCount = getVertexCount(translucentData);
            int quadCount = vertexCount / 4;

            if (quadCount > 0) {
                long[] sortingData = new long[quadCount];
                long src = getVertexDataAddress(translucentData);
                ...
                for (int j = 0; j < vertexCount; j++) {
                    ...
                    if ((j & 3) == 3) {
                        ...
                        long packedSortingData = (((long) sortDistance) << 32) | (quadId);
                ...
                for (int i = 0; i < sortingData.length; i++) {
                    ...
                    copyQuad(src + srcQuadIndex * 4L * formatSize, outPtr + ((sortingData.length - 1) - i) * 4L * formatSize);
                }
        ...
        if (solid != null) {
            int vertexCount = getVertexCount(solid);
            if (vertexCount > 0) {
                long src = getVertexDataAddress(solid);
        ...
        // Process cutout geometry
        if (cutout != null) {
            int vertexCount = getVertexCount(cutout);
            if (vertexCount > 0) {
                long src = getVertexDataAddress(cutout);
                long dst = outPtr + offset * 4L * formatSize;
                MemoryUtil.memCopy(src, dst, (long) vertexCount * formatSize);
